 ...
 * setup-waf.sh uses this sidecar so the lab can still create a Classic WebACL.
 */
...
function handlers(op, input) {
  switch (op) {
    ...
    case 'GetByteMatchSet':
      return { ByteMatchSet: db.byteSets.get(input.ByteMatchSetId) };

    case 'CreateSizeConstraintSet': {
      const item = { SizeConstraintSetId: id(), Name: input.Name, SizeConstraints: [] };
      db.sizeSets.set(item.SizeConstraintSetId, item);
      return { SizeConstraintSet: item, ChangeToken: token() };
    }
    case 'UpdateSizeConstraintSet': {
      const item = db.sizeSets.get(input.SizeConstraintSetId);
      if (!item) throw Object.assign(new Error('SizeConstraintSet not found'), { http: 400 });
      for (const u of input.Updates || []) {
        if (u.Action === 'INSERT' && u.SizeConstraint) item.SizeConstraints.push(u.SizeConstraint);
      }
      return { ChangeToken: token() };
    }
    ...
    case 'ListWebACLs':
      return {
        WebACLs: [...db.webAcls.values()].map((w) => ({
          WebACLId: w.WebACLId,
          Name: w.Name,
        })),
      };
    ...
    case 'ListSizeConstraintSets':
      return {
        SizeConstraintSets: [...db.sizeSets.values()].map((s) => ({
    ...
    try {
      ...
      if (!out) {
        return err(
          ...
          400,
          'InvalidAction',
          `Unsupported WAF Classic operation: ${op || target || req.url}`
        );
      }
      return ok(res, out);
    } catch (e) {
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('[waf-classic] LocalStack Community does not implement this service (Pro-only)');
});
