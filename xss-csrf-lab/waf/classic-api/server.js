...
function handlers(op, input) {
  switch (op) {
    ...
    case 'CreateXssMatchSet': {
      const item = { XssMatchSetId: id(), Name: input.Name, XssMatchTuples: [] };
      db.xssSets.set(item.XssMatchSetId, item);
      return { XssMatchSet: item, ChangeToken: token() };
    }
    case 'UpdateXssMatchSet': {
      const item = db.xssSets.get(input.XssMatchSetId);
      if (!item) throw Object.assign(new Error('XssMatchSet not found'), { http: 400 });
      for (const u of input.Updates || []) {
        if (u.Action === 'INSERT' && u.XssMatchTuple) item.XssMatchTuples.push(u.XssMatchTuple);
      }
      return { ChangeToken: token() };
    }
    case 'GetXssMatchSet':
      return { XssMatchSet: db.xssSets.get(input.XssMatchSetId) };
    ...
    case 'ListXssMatchSets':
      return {
        XssMatchSets: [...db.xssSets.values()].map((s) => ({
